...
M.extend(M.Page.DRTrans.prototype, {
    ...
    inf: {},
    transtype: "DR",
    tpl_divinfo: '<div class="date-have">' + '<div class="${css}" style="width:${width}px">' + '<p class="sortorder">${guestname}</p>' + '<p class="t12 light sortorder" >${channel}</p>' + '<div class="demand"></div>' + '</div>' + '</div>',
    tpl_divinfors: '<div class="date-have">' + '<div class="${css}" style="width:${width}px">' + '<p class="sortorder">${guestname}</p>' + '</div>' + '</div>',
    init: function() {
        this.initDOM();
        this.initEvent();
        this.win_resize();
    ...
    _resettotal: function(tpl_room, rtid) {
        ...
        tpllist.each(function() {
            ...
            $(this).children("div").removeClass("full-room");
            if (html == 0) {
                html = '满房';
                $(this).children("div").addClass("full-room");
            }
    ...
    detectform_click: function(e) {
        ...
        var t = ele.attr("tag");
        if (t == "close") {
            this.context.detectform.animate({
                height: '0'
            },
            300);
    ...
    showPushScroll: function() {
        var height = $(document).height() - 47;
        $('#otamsgbox').css('maxHeight', height + 'px');
    },
        ...
        if (tag == "left" && style.indexOf("prev_no") == -1) {
            ...
            if (margin_left == 0) {
                ...
                if ((margin_left - 500) < -this.innmorewidth) {
                    ...
                    tpl.animate({
                        "margin-left": margin_left - scrolllength + "px"
                    },
                    "slow");
                    this.context.footer.children("div").children("a").removeClass("prev_no");
                    ele.addClass("prev_no");
    ...
    scroll_right: function() {
        ...
        this.context.pickerarea.animate({
    ...
    getdata_bycoordinate: function(row, col) {
        ...
        if (!M.isEmpty(row)) {
            var target = this.context.roomlist.children("tbody").children("tr[i=" + row + "]");
            data.roomid = target.attr("rid");
            data.roomname = target.find("div").children("span").html();
            data.rtid = target.attr("rtid");
            data.roomtypename = target.children("td").attr("rtname");
            data.type = this.context.roomlist.children("tbody").children("tr").children("td[tag=watertotal][i=" + row + "]").attr("type");
            var key = 'r_' + row + "c_" + col;
            if (!M.isEmpty(this.targetpricelist[key])) {
                data.price = this.targetpricelist[key];
            } else {
                data.price = this._getsetprice(data);
            }
        }
        return data;
    },
    _getsetprice: function(data) {
        var price = 0;
        if (!M.isEmpty(this.pricelist[data.rtid])) {
            if (!M.isEmpty(this.pricelist[data.rtid]['pricelist'])) {
                if (!M.isEmpty(this.pricelist[data.rtid]['pricelist'][data.date])) {
                    price = this.pricelist[data.rtid]['pricelist'][data.date]['price'];
                }
            }
        }
        return price;
    ...
    _getjump_selected: function(idx, orderdata) {
        ...
        var roomlist = this.context.roomlist.children("tbody");
        var datelist = this.context.pickerdate;
        ...
        tpl.each(function() {
            ...
            var tpl_room = roomlist.children("tr[i=" + idx + "]");
            var roomid = tpl_room.attr("rid");
            var roomname = tpl_room.find("div").html();
            var rtid = tpl_room.children("td").attr("rtid");
            var date = datelist.find("td[i=" + i + "]").attr("time");
            if (M.isEmpty(price)) {
                price = 0;
            }
            if (!M.isEmpty(pricelist[rtid]) && price == 0) {
                if (!M.isEmpty(pricelist[rtid]['pricelist'])) {
                    if (!M.isEmpty(pricelist[rtid]['pricelist'][date]['price'])) {
                        price = pricelist[rtid]['pricelist'][date]['price'];
                ...
                for (var j = 0; j < orderdata.length; j++) {
                    ...
                    if (data.idx == idx) {
                        if ((i - data.end_i * 1) == 1) {
                            ...
                            data.price = parseInt(data['price']) + parseInt(price);
                            orderdata[j] = data;
                            has = 1;
                        }
                    }
    ...
    _getroomtotal: function() {
        var leftroom = {};
        var tpl = this.context.pickerarea.children("table").children("tbody").children("tr[tag=o]").children("td");
        tpl.each(function() {
            var idx = $(this).attr("idx");
    ...
    resetroomamount_remain: function() {
        ...
        this.context.pickerdate.children("table").children(":first").children("tr").children().each(function() {
            var i = $(this).attr("i");
    ...
    changeorderstatus_moredays: function(target, nights, type, cols) {
        ...
        var nights = target.attr("n");
        var rid = target.children("div").attr("rid");
        var oid = target.children("div").attr("oid");
        var gid = target.children("div").attr("gid");
        for (var j = 0; j < nights - 1; j++) {
            target = target.next();
            target.html("<div style=''></div>").attr("sta", type);
            target.parents("td").attr("sta", type).attr("setid", setid).attr("n", nights);
            target.children("div").addClass("date-have").attr("gid", gid).attr("oid", oid).attr("setid", setid).attr("n", 1).attr("sta", "ordered");
        }
    },
    hightrow: function(e) {
        var ele = M.EventEle(e);
    ...
    hightcol: function(e) {
        ...
        if (!M.isEmpty(tag) && tag == 'roomtf') return;
    ...
    canshiftedorders: function(ele, rows, sorttable) {
        ...
        var target_oid = ele.attr("oid");
        var target_cid = ele.attr("cid");
        if (sta == 'checkedout') {
            return false;
        }
        var needcheck = false;
        if ((sta == "ordered" && sorttable.oid != target_oid)) {
            needcheck = true;
        } else if (sta != "ordered" && sorttable.cid != target_cid) {
            needcheck = true;
        }
        if (needcheck) {
            var checkfirst = this.checkisfirsttd(ele);
            if (!checkfirst) {
                return false;
            }
        }
        ...
        for (i; i <= range; i++) {
            ...
            if (i == start) {
                ...
                if (!M.isEmpty(oid) || !M.isEmpty(cid)) {
                    if (((M.isEmpty(targetoid) && oid != sorttable.oid) || (!M.isEmpty(cid) && cid != sorttable.cid && M.isEmpty(targetcid))) || (!M.isEmpty(targetoid) && target_oid != oid) || (!M.isEmpty(targetcid) && target_cid != cid)) {
                        return false;
                    }
                    if (has == 1) {
                        return false;
    ...
    checkisfirsttd: function(ele) {
        var targettd = ele.parents("td");
        var sta = targettd.attr("sta");
        var n = targettd.attr("n");
        var oid = targettd.children("div").attr("oid");
        var cid = targettd.children("div").attr("cid");
        var idx = targettd.attr("idx");
        var i = targettd.parent().attr("i");
        if (i == 0 || sta == 'unordered') {
            return true;
        }
        idx = parseInt(idx) - 1;
        var pretpl = targettd.parents('tr').children("td[idx=" + idx + "]");
        var pr_sta = pretpl.attr("sta");
        if (pr_sta == 'unordered' || M.isEmpty(pr_sta)) {
            return true;
        }
        var setid = pretpl.attr("setid");
        var pre_oid = pretpl.children("div").attr("oid");
        var pre_cid = pretpl.children("div").attr("cid");
        if (sta == "ordered") {
            if (oid != pre_oid) {
                return true;
            }
    ...
    _getorderwidth: function() {
        return this.context.pickerdate.find("td").width();
    },
    clearitem: function(col, row, length) {
        var target = col * 1 + length * 1 - 1;
        var orderwidth = this._getorderwidth();
        for (var i = col; i <= target; i++) {
            ...
            if (this.inf.ov == '1' && this.inf.tp == '1') {
                ...
                tpl.children("div").attr("title", '').tooltip({
                    position: {
                        my: "left+15 top+20",
                        at: "left bottom"
                    },
            ...
            tpl.children("div").css("width", orderwidth).attr("sta", "unordered").attr("oid", "").attr("cid", "").attr("n", "").attr("hidedays", "").attr("setid", "").attr("rid", "").removeClass("date-have").addClass("date-day");
        }
    },
    ...
    getroomlist: function() {
        return this.context.roomtype.find("tr[tag=roomtype]").children("td");
    },
    resizeorder: function(target) {
        var nights = parseInt(target.children("div").children("div").attr("n"));
        var width = 80;
        if (this.reducedstate == 1) {
            width = '80';
        }
        var orderwidth = width * nights - 6 + nights - 1;
        target.children("div").children("div").css("width", orderwidth);
    },
    enlargeordersize: function(target) {
        var nights = parseInt(target.children("div").children("div").attr("n"));
        var width = 80;
        if (this.reducedstate == 1) {
            width = '80';
        }
        target.children("div").children("div").css("width", width * nights + nights - 1);
    }
});
...
M.extend(M.Page.RDTrans.prototype, {
    ...
    addcolumn: 0,
    orderwidth: 0,
    tpl_divinfo: '<div class="date-have">' + '<div class="${css}" style="height:${height}px">' + '<p class="sortorder">${guestname}</p>' + '<p class="t12 light sortorder" >${channel}</p>' + '<div class="demand"></div>' + '</div>' + '</div>',
    tpl_divinfors: '<div class="date-have">' + '<div class="${css}" style="height:${height}px">' + '<p class="sortorder">${guestname}</p>' + '</div>' + '</div>',
    width: 0,
    scroll_width: 0,
    reducedstate: 0,
    targetpricelist: {},
    ...
    win_resize: function() {
        this.context.scroller.width($(window).width() - 136 - 1);
        ...
        if (isfold == 1) {
            var roomcount = this.context.roomlist.find("tr[tag=rooms]").children("td:visible").length;
            this.orderwidth = this.context.roomlist.find("tr[tag=rooms]").width();
        } else {
            var roomcount = this.context.roomlist.find("td[tag=room]").length;
            this.orderwidth = this.context.roomlist.find("tr[tag=room]").width();
        }
        var neww = 0;
        if (this.reducedstate == 1) {
            this.context.roomlist.attr("style", "").parent().css("overflow-y", this.context.scroller.css("overflow-y"));
            var overflowx = this.context.scroller.css("overflow-x");
            this.context.scroll_left.css("overflow-x", this.context.scroller.css("overflow-x"));
        } else {
    ...
    _getjump_selected: function(idx, orderdata) {
        ...
        var roomlist = this.context.roomlist.children("tbody").children("tr");
        var datelist = this.context.pickerdate;
        var pricelist = this.pricelist;
        tpl.each(function() {
            var has = 0;
            ...
            var date = datelist.find("tr[i=" + i + "]").attr("time");
            var tpl_room = roomlist.children("td[tag=room][i=" + idx + "]");
            var roomid = tpl_room.attr("rid");
            var roomname = tpl_room.children("div").html();
            var rtid = tpl_room.attr("rtid");
            var price = $(this).attr("price");
            if (M.isEmpty(price)) {
                price = 0;
            }
    ...
    getdata_bycoordinate: function(row, col) {
        ...
        if (!M.isEmpty(col)) {
            var target = this.context.roomlist.children("tbody").children("tr").children("td[tag=room][i=" + col + "]");
            data.roomid = target.attr("rid");
            data.roomname = target.children("div").children("span").html();
            data.roomtypename = target.attr("rtname");
            data.rtid = target.attr("rtid");
    ...
    getcoordinate_bydata: function(date, roomid, type) {
        ...
        if (!M.isEmpty(date)) {
            data.date_i = this.context.pickerdate.children("table").find("tr[time=" + date + "]").attr("i");
        }
        if (!M.isEmpty(type)) {
            data.type_i = this.context.roomlist.children("tbody").children("tr").children("td[tag=watertotal][type=" + type + "]").attr("i");
        }
        if (!M.isEmpty(roomid)) {
            data.roomid_idx = this.context.roomlist.find("td[rid=" + roomid + "]").attr("i");
        }
        return data;
    },
    ...
    getcellbycoordinate: function(i, idx) {
        if (!M.isEmpty(idx)) {
            return this.context.roomlist.children("tbody").find("tr>td[i=" + idx + "][tag=room]:first");
        }
        if (!M.isEmpty(i)) {
    ...
    resetroomamount_remain: function() {
        ...
        this.context.pickerarea.children("table").children(":first").children("tr:visible").each(function() {
            var len = $(this).children("td[sta=unordered]").length;
            var r = $(this).attr("i");
            leftroom[r] = len;
        });
        var pickerarea = this.context.pickerarea;
        this.context.pickerdate.children("table").children(":first").children().each(function() {
            var i = $(this).attr("i");
    ...
    changeorderstatus_moredays: function(target, nights, type, cols) {
        ...
        for (var j = 0; j < nights - 1; j++) {
            ...
            target.html("<div style=''></div>").attr("sta", type);
            target.attr("sta", type).attr("setid", setid).attr("n", nights);
            target.children("div").addClass("date-have").attr("sta", type).attr("rid", rid).attr("oid", oid).attr("gid", gid).attr("setid", setid).attr("n", 1);
        }
    },
    hightrow: function(e) {
        var ele = M.EventEle(e);
    ...
    hightcol: function(e) {
        ...
        var tag = ele.parent().attr("tag");
        if (tag == 'roomtype' || tag == 'roomcount') return;
        var selected = this.context.pickerarea.children("table").children("tbody").find("td[idx=" + x + "]").attr("class");
        this.selectedreset();
        if (typeof(selected) != 'undefined' && (selected.indexOf('selected') > 0 || selected == 'selected')) {
            return;
    ...
    checkisfirsttd: function(ele) {
        ...
        i = parseInt(i) - 1;
        var pretpl = targettd.parents('tbody').children("tr[i=" + i + "]").children("td[idx=" + idx + "]");
        var pr_sta = pretpl.attr("sta");
        if (pr_sta == 'unordered' || M.isEmpty(pr_sta)) {
            return true;
        }
    ...
    _getorderwidth: function() {
        var target = this.context.pickerarea.find("td[tag=day][sta=unordered]").children("div");
        if (target.length == 0) {
            return this.context.pickerarea.find("td[tag=day][n=1]").children("div").width();
        } else {
            if (target.width() == 0) {
                return this.context.pickerarea.find("td[tag=day][sta=unordered]").width();
            }
            return target.width();
        }
    },
    clearitem: function(col, row, length) {
        ...
        for (var i = row; i <= target; i++) {
            ...
            tpl.attr("sta", "unordered").attr("n", "").attr("hidedays", "").attr("setid", "");
            tpl.children("div").css("width", orderwidth).attr("oid", "").attr("cid", "").attr("sta", "unordered").attr("n", "").attr("hidedays", "").attr("setid", "").attr("rid", "").removeClass("date-have").addClass("date-day");
        }
    },
    _clearitembyset: function(targets, type) {
        var len = targets.length;
    ...
    _changecloroomtotal: function(rtid) {
        ...
        for (var i = 0; i <= rowlength; i++) {
            ...
            target.find("td[tag=c][rtid=" + rtid + "]").children().removeClass("full-room");
            if (roomlength == 0) {
                roomlength = '满房';
                target.find("td[tag=c][rtid=" + rtid + "]").children().addClass("full-room");
            }
    ...
    _changefoldstatus: function(rtid, showstatus) {
        ...
        var roomtypelist = this.context.roomlist.find("tr[tag=roomtypes]").children("td[rtid=" + rtid + "]");
        var roomlist = this.context.roomlist.find("tr[tag=rooms]");
        var pickerarea = this.context.pickerarea;
        var neww = this._getorderwidth();
        if (!M.isEmpty(neww)) {
            roomtypelist.each(function() {
                var rtid = $(this).attr("rtid");
                var roomlength = roomlist.children("td[rtid=" + rtid + "]").not("td:hidden").length;
                $(this).children("div").css("width", roomlength * neww + "px");
                roomlist.children("td[rtid=" + rtid + "]").children("div").css("width", neww + "px");
                pickerarea.find("td[tag=c][rtid=" + rtid + "]").children("div").css("width", neww + "px");
            });
        }
    },
    ...
    getroomlist: function() {
        return this.context.roomtype.find("td[tag=room]");
    },
    resizeorder: function(target) {
        var nights = parseInt(target.children("div").children("div").attr("n"));
        var height = 46;
        ...
        var orderheight = height * nights + -6;
        target.children("div").children("div").css("height", orderheight);
    },
    enlargeordersize: function(target) {
        var nights = parseInt(target.children("div").children("div").attr("n"));
        var height = 46;
        ...
        target.children("div").children("div").css("height", height * nights);
    }
});
...
M.extend(M.Page.DatePickerTransform.prototype, {
    ...
    getroomlist: function() {
        return this.transobj.getroomlist();
    },
    resizeorder: function(target) {
        return this.transobj.resizeorder(target);
    },
    enlargeordersize: function(target) {
        return this.transobj.enlargeordersize(target);
    },
    _getorderwidth: function() {
        return this.transobj._getorderwidth();
    },
    _setpricelist: function(pricelist) {
        this.transobj.pricelist = pricelist;
    }
});
