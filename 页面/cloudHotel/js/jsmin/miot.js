...
jQuery.stringifyJSON = function(b) {
    if (window.JSON && window.JSON.stringify) {
        return window.JSON.stringify(b);
    }
    switch (jQuery.type(b)) {
    case "string":
        return '"' + b.replace(/[\x00-\x1f\\"]/g, escape) + '"';
    case "array":
        return "[" + jQuery.map(b, jQuery.stringifyJSON) + "]";
    case "object":
        var a = [];
        jQuery.each(b,
        function(d, e) {
            var c = jQuery.stringifyJSON(e);
            if (c) {
                a.push(jQuery.stringifyJSON(d) + ":" + c);
            }
        });
        return "{" + a + "}";
    case "number":
    ...
    M.extend(M, {
        ...
        jsonToStr: function(oJson) {
            ...
            if (typeof(oJson) == typeof(0)) return oJson.toString();
        ...
        timeformat: function(date, format) {
            ...
            var time = format.replace("Y", year).replace("m", month).replace("d", day).replace("h", hours).replace("i", minis).replace("s", second);
            return time;
        },
        ...
        _getjson: function(url, data, rescallback, methodtype) {
            ...
            $.ajax({
                ...
                success: function(e) {
                    ...
                    res.req = data;
                    if (!M.isEmpty(res.status) && res.status == "appnotlogin") {
                        if (M.isEmpty(M.timeout)) {
                            M.timeout = 1;
                            alert("您已经很长时间没有操作了，请重新登录");
                        }
                        window.top.location.href = "yzg://login.miot.cn";
                        return;
                    }
                ...
                error: function() {
                    if (rescallback != null) {
                        rescallback.call(rescallback.context, {
                            status: "ERROR"
                        });
                    }
                }
        ...
        jumpnewpage: function(url) {
            window.location.href = url;
        },
        openyzgoapp: function(url) {
            var hotelchange = $.cookie('hotelchange');
            if (!M.isEmpty(hotelchange) && hotelchange == 1) {
                url += '/hotelchange/' + hotelchange;
            }
            var opencard = $.cookie('opencard');
            var cookiejson = $.cookie.json;
            $.cookie.json = false;
            if (!M.isEmpty(opencard) && opencard == 1) {
                var cookie = $.cookie('cookiekey');
                url += '/cookiekey/' + cookie;
            }
...
})(jQuery); (function($, document, undefined) {
    var pluses = /\+/g;
    function raw(s) {
        return s;
    }
    ...
    var config = $.cookie = function(key, value, options) {
        if (value !== undefined) {
            ...
            return (document.cookie = [encodeURIComponent(key), '=', config.raw ? value: encodeURIComponent(value), options.expires ? '; expires=' + options.expires: '', options.path ? '; path=' + options.path: '', options.domain ? '; domain=' + options.domain: '', options.secure ? '; secure': ''].join(''));
        }
...
M.extend(M.Controls.DropdownList.prototype, {
    ...
    init: function(ele, onchange, options) {
        ...
        this.listdom.attr("tag", "dropdownlist") this.valele = this.listdom.children("span");
    ...
    _toggle: function(ele) {
        var status = ele.children("div").css("display");
        ele.children("div").toggle();
        if (status == "none") {
            var optionheight = 30;
            var value = ele.children("span").attr("value");
            var maxheight = ele.children("div").children("div").css("max-height").replace("px", "");
            if (M.isEmpty(maxheight)) {
                maxheight = ele.children("div").css("max-height").replace("px", "");
                ele.children("div").scrollTop(0);
            }
            if (!M.isEmpty(maxheight) && !M.isEmpty(value)) {
                ele.children("div").children("div").scrollTop(0);
                var endtop = ele.find("div[tag=option][value=" + value + "]").offset().top;
                var starttop = ele.children("div").children("div").offset().top;
                var rangeheight = ele.children("div").children("div").height();
                var prangheight = ele.children("div").height();
                if (rangeheight > prangheight) {
                    rangeheight = prangheight;
                }
                var mheight = endtop - starttop + 30;
                if (mheight >= rangeheight) {
                    var scrollheight = mheight - maxheight + 100;
                    if (scrollheight < 0) {
                        scrollheight = 0;
                    }
                    ele.children("div").children("div").scrollTop(scrollheight);
                }
            }
        }
    },
...
M.Controls.H5 = M.createClass();
M.extend(M.Controls.H5.prototype, {
    tpl: '<div style="display:none;" id="h5_ui_message" class="g-pop">' + '<div class="mask"></div>' + '<div class="ui_messageBox">' + '<div class="content" tag="msg">操作失败</div>' + '<div style="display:none;" tag="op" class="op">' + ' <a class="btn" href="javascript:;" tag="confirm">是</a>' + '<a class="btn" href="javascript:;" tag="cancle">否</a>' + ' </div>' + '<div class="op op-1" tag="close">' + '<a class="btn btn-primary" href="javascript:void(0);" tag="confirm">知道了</a>' + '</div>' + '</div>' + '</div>',
    uimessage_init: function(msg) {
        $(document.body).append(this.tpl);
        var ui_messageBox = $("#h5_ui_message");
        ui_messageBox.show().find("div[tag=msg]").html(msg);
        return ui_messageBox;
    },
    success: function(msg) {
        var ui_messageBox = this.uimessage_init(msg);
        ui_messageBox.find("a[tag=confirm]").bind("click", this.closeMessage);
    },
    error: function(msg) {
        var ui_messageBox = this.uimessage_init(msg);
        ui_messageBox.find("a[tag=confirm]").bind("click", this.closeMessage);
    ...
    confirmmessage: function(msg, confirm_func, domsg) {
        ...
        if (typeof(eval(confirm_func)) == "function") {
            ui_messageBox.find("a[tag=confirm]").bind("click", confirm_func);
        } else {
            ui_messageBox.find("a[tag=confirm]").bind("click", this.closeMessage);
        }
    },
    confirm: function(confirm_func, cancle_func, msg) {
        var ui_messageBox = this.uimessage_init(msg);
        ui_messageBox.find("div[tag=op]").show();
        ui_messageBox.find("div[tag=close]").hide();
        ui_messageBox.find("div[tag=op]").children("a[tag=confirm]").bind("click", confirm_func);
        ui_messageBox.find("div[tag=op]").children("a[tag=cancle]").bind("click", cancle_func);
    },
    closeMessage: function() {
        $("#h5_ui_message").remove();
    }
});
M.H5 = new M.Controls.H5();
M.extend(M, {
    ...
    confirm_tpl: '<div id="ui_message" class="ui_messageBox fade in ui_mb_success" style="display:none;"><div class="content" tag="msg"></div><div class="mt20">' + '<a href="javascript:;" class="mr20 btn btn-primary" tag="confirm">确认</a><a href="javascript:;" tag="cancle" class="btn">取消</a></div></div>',
    confirmmessage_tpl: '<div id="ui_message" class="ui_messageBox fade in ui_mb_success" style="display:none;"><div class="content" tag="msg"></div><div class="mt20">' + '<a href="javascript:;" tag="confirm" class="btn">确认</a></div></div>',
    getVal: function(ele) {
    ...
    uimessage_init: function(layer_tpl, message_tpl, msg) {
        ...
        if (!M.isEmpty(msg)) ui_messageBox.children("div[tag=msg]").html(msg);
    ...
    closeMessage: function() {
        var ui_messageBox = $("#ui_message");
        var transparentlayer = $("#lightgraylayer");
        ui_messageBox.hide("3000",
        function() {
            ui_messageBox.removeClass('in');
            transparentlayer.hide();
        });
        transparentlayer.hide("1000",
        function() {
            ui_messageBox.hide();
        });
...
$(document).ready(function() {
    ...
    var tool = $('#header .tool a') var tool_active = $('#header .tool .active') tool.bind('mouseover',
    function() {
        if ($(this).hasClass('active')) {} else {
            $(this).addClass('hover') tool_active.removeClass('hover')
        }
    });
    tool.bind('mouseout',
    function() {
        $(this).removeClass('hover') tool_active.addClass('hover')
    });
