...
M.extend(M.Page.ReportRoom.prototype, {
    ...
    dropdownliststa: {},
    submittext: "处理中...",
    tpl_consume: '<tr id="${id}" orderstatus="${orderstatus}">' + '<td>${guestname}</td>' + '<td>${roominfo}</td>' + '<td>${consumedate}</td>' + '<td>${checkoutdate}</td>' + '<td>${consumename2}</td>' + '<td>${consumename1}</td>' + '<td>${amount}</td>' + '<td>${orderstatusname}</td>' + '<td><a href="#?" tag="del" class="ml10">删除</a></td>' + '</tr>',
...
function showincomedetail(incomesource) {
    ...
    for (var i = 0; i < incomesource.length; i++) {
        ...
        var val = parseInt(income.income);
        total += val;
...
function _convertdatasource(source) {
    ...
    for (var i = 0; i < source.length; i++) {
        ...
        formatedsource.push({
            "ondate": M.strtotime(item.ondate),
            "sales": item.sales,
            "roomnum": parseInt(item.roomnum),
            "realroomnum": parseInt(item.realroomnum),
            'datestr': item.ondate
        });
    }
    ...
    if (1 == 1) {
        ...
        if (!M.isEmpty(source)) {
            ...
            for (var k in source) {
                ...
                if (t == '1') {
                    ydata.push(parseInt(item.sales));
                    var caption = '营业收入';
                    var unit = ' 元';
                } else if (t == '2') {
