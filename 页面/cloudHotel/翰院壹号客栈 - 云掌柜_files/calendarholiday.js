...
if(lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)){tmp1=Number(RegExp.$1)-lDObj.month
tmp2=Number(RegExp.$2)-parseInt(lDObj.day)
if(tmp1==0&&tmp2==0)
lunarFestival=RegExp.$4}
for(i in sFtv)
if(sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/)){tmp1=Number(RegExp.$1)-(SM+1)
...
return(0)},leapMonth:function(y){return(this.lunarInfo[y-1900]&0xf)},monthDays:function(y,m){return((this.lunarInfo[y-1900]&(0x10000>>m))?30:29)}});M.extend(M,{Holiday:function(ele,options){var control=new M.Controls.Holiday();return control;}});