...
return f;};M.extend(M,{timeout:null,namespace:function(ns_string){var parts=ns_string.split('.'),parent=M,i;if(parts[0]==="M"){parts=parts.slice(1);}
...
return true;},_formatenum:function(data){if(!this.math_isnumber(data)&&typeof(data)=='string'){if(data.indexOf('.')>=0){var list=data.split('.');return list[0]+'.'+list[1];}}
...
res.req=data;if(rescallback!=null){rescallback.call(tempcontext,res);}},error:function(){if(rescallback!=null){rescallback.call(tempcontext,{status:"ERROR"},data);}}});},_getjson:function(url,data,rescallback,methodtype){var reqUrl=url;if(url.indexOf("?")>-1){reqUrl+="&t="+new Date().getMilliseconds();}
...
if(rescallback!=null){rescallback.call(rescallback.context,res);}},error:function(){if(rescallback!=null){rescallback.call(rescallback.context,{status:"ERROR"});}}});},_gethtml:function(url,data,rescallback,callbackcontext){var that=this;var tempcontext=callbackcontext;if(tempcontext==undefined||tempcontext==null||tempcontext==''){tempcontext=that;}
var reqUrl=url;if(url.indexOf("?")>-1){reqUrl=url+"&"+M.jsonToQueryStr(data)+"&t="+new Date().getMilliseconds();}
else{reqUrl=url+"?"+M.jsonToQueryStr(data)+"&t="+new Date().getMilliseconds();;}
...
if(rescallback!=null){rescallback.call(tempcontext,res);}},error:function(){if(rescallback!=null){rescallback.call(tempcontext,{status:"ERROR"});}}});},isArray:function(obj){return Object.prototype.toString.call(obj)==='[object Array]';},in_array:function(value,arr){if(!M.isArray(arr)){alert('not Array');return false;}
...
tool_active.addClass('hover')});$("a[tag=closebtn]").click(function(){M.CloseLast();M.stopevent();}.toEventHandler(this));$(".main-user").hover(function(){$(this).children('p').show();},function(){$(this).children('p').hide();});$(".main-user").click(function(){location.href='/Logout'});});